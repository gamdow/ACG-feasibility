...
static bool isbuilt_wave_block0_4_kernel = false;

void buildOpenCLKernels_wave_block0_4_kernel(int xdim0, int xdim1) {

  //int ocl_fma = OCL_FMA;
  if(!isbuilt_wave_block0_4_kernel) {
    buildOpenCLKernels();
    //clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char* source_filename[1] = {(char*)"./OpenCL/wave_block0_4_kernel.cl"};
    ...
    printf("Compiling wave_block0_4_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_wave_block0_4_kernel=%d  -Dxdim1_wave_block0_4_kernel=%d ", pPath, 32,xdim0,xdim1);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_wave_block0_4_kernel=%d  -Dxdim1_wave_block0_4_kernel=%d ", pPath, 32,xdim0,xdim1);
      else {
      ...
      printf("compiling wave_block0_4_kernel -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[1] = clCreateKernel(OPS_opencl_core.program, "ops_wave_block0_4_kernel", &ret);
    clSafeCall( ret );

    isbuilt_wave_block0_4_kernel = true;
  }
...
// host stub function
void ops_par_loop_wave_block0_4_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (OPS_diags > 1) {
    ops_timing_realloc(1,"wave_block0_4_kernel");
    OPS_kernels[1].count++;
  ...
  buildOpenCLKernels_wave_block0_4_kernel(
  xdim0,xdim1);
