...
int main (int argc, char **argv) 
{
   ...
   ops_block wave_block;

   // Initialising block in OPS Format
   wave_block = ops_decl_block(1, "wave_block");
   ...
   int base[] = {0};
   double* val = NULL;
   wk0 = ops_decl_dat(wave_block, 1, size, base, halo_m, halo_p, val, "double", "wk0");
   wk1 = ops_decl_dat(wave_block, 1, size, base, halo_m, halo_p, val, "double", "wk1");
   phi_old = ops_decl_dat(wave_block, 1, size, base, halo_m, halo_p, val, "double", "phi_old");
   phi = ops_decl_dat(wave_block, 1, size, base, halo_m, halo_p, val, "double", "phi");
   ...
   ops_par_loop(wave_block0_5_kernel, "Initialisation", wave_block, 1, iter_range5,
   ...
   for (int iteration=0; iteration<2500; iteration++){
      ...
      ops_par_loop(wave_block0_4_kernel, "Save equations", wave_block, 1, iter_range4,
      ...
      for (int stage=0; stage<3; stage++){
         ...
         ops_par_loop(wave_block0_0_kernel, "D(phi x0)", wave_block, 1, iter_range0,
         ops_arg_dat(phi, 1, stencil0, "double", OPS_READ),
         ops_arg_dat(wk0, 1, stencil1, "double", OPS_WRITE));


         int iter_range1[] = {0, nx0};
         ops_par_loop(wave_block0_1_kernel, "Residual of equation", wave_block, 1, iter_range1,
         ops_arg_dat(wk0, 1, stencil1, "double", OPS_READ),
         ops_arg_dat(wk1, 1, stencil1, "double", OPS_WRITE));


         int iter_range2[] = {-4, nx0 + 4};
         ops_par_loop(wave_block0_2_kernel, "RK new (subloop) update", wave_block, 1, iter_range2,
         ...
         ops_par_loop(wave_block0_3_kernel, "RK old update", wave_block, 1, iter_range3,
         ops_arg_dat(wk1, 1, stencil1, "double", OPS_READ),
   ...
   ops_fetch_block_hdf5_file(wave_block, "wave_2500.h5");
   ops_fetch_dat_hdf5_file(phi, "wave_2500.h5");
